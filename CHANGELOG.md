...
- **Stateful MTP 投机解码（Speculative Decoding）**：`LlamaSpecEngine` 生命周期管理 + `SpecConfig` / `SpeculativeType` 正式落地；**BREAKING**：移除旧 `LlamaPromptLookupDecoding`，NGram 走新生命周期。推荐 Qwen3.8 27B 起手 `draft_n_max=2`；MTP 目前仅文本 / 单序列。
- **fix(mtmd) ctypes 指针绑定修正**：`unsigned char *` 从 `c_char_p` 改为 `POINTER(c_uint8)` —— 上游已修复此前需手动 workaround 的视觉/音频缓冲区坑。
...
`GenericMTMDChatHandler.__init__` 从 0.3.47 的 `clip_model_path=` 变为 0.3.48 的：

```python
GenericMTMDChatHandler(chat_format, mmproj_path, verbose=True, ...)
```

- `chat_format` 可为 `None`（自动解析），`mmproj_path` 为**必填位置参数**。
- 下游插件 / 自定义脚本作者**必须跟进此变更**，否则构造期直接报缺位置参数。
- 视觉 handler 类（`Qwen3VLChatHandler` / `Qwen25VLChatHandler` / `GenericMTMDChatHandler`）及音频 handler（`Qwen3ASRChatHandler`）在本构建下均验证可正常导入。

### ⚠️ 已知集成注意：hybrid 视觉模型 + `ctx_checkpoints=0` 首 decode 崩溃

- 现象：Qwen3.5 等带 SWA 层的 hybrid 视觉模型，在大图（约 4000+ vision tokens）下，prefill 正常但**首 decode token 崩溃**（`failed to prepare attention ubatches` / `failed to find a memory slot for batch of size 1`）。
- 根因：调用方传 `ctx_checkpoints=0` 会强制 hybrid 模型走一条 "Bypassing rollback" fast-path，该路径在大 prefill 下无 KV / 解码槽余量给首 decode token。**此问题在 0.3.48+ 稳定暴露**（更早版本同参数组合未稳定触发）。
- 规避：`ctx_checkpoints` 用默认 `-1`（llama_cpp 内部映射为 16，启用 checkpoint 缓存 → 避开该缺陷分支）。ComfyUI-sg-llama-cpp fork 已在 commit `1f0fc15` 将默认改为 `-1` 并加响应式 `n_ctx` hint。
- **本 wheel 本身无此 bug**：纯 `llama_cpp.Llama` 同模型同大图在 `n_ctx=8192` 下正常（已双验证），崩点仅出现在特定插件调用姿势。
...
- **构建环境**：oneAPI 2026.1 + SYCL（icx，INTEL target），Python 3.13.11，scikit-build-core 0.10.7。
...
- **音频生成接口目前仅绑定级暴露**：Pocket TTS 等 MTMD 音频 API 为实验性绑定适配，高层 Python 工作流尚未集成。
